...
test.describe('Queue Management', () => {
  test('should fetch queue summary', async ({ request }) => {
    const response = await request.get('https://localhost:21371/api/queue/summary', {
  ...
  test('should fetch queue statistics', async ({ request }) => {
    const response = await request.get('https://localhost:21371/api/queue/stats', {
      ignoreHTTPSErrors: true
    });
  ...
  test('should fetch queue items with pagination', async ({ request }) => {
    const response = await request.get('https://localhost:21371/api/queue/items?limit=10&offset=0', {
      ignoreHTTPSErrors: true
    });
  ...
  test('should filter queue items by status', async ({ request }) => {
    const response = await request.get('https://localhost:21371/api/queue/items?status=pending', {
      ignoreHTTPSErrors: true
    });
  ...
  test('should get worker status', async ({ request }) => {
    const response = await request.get('https://localhost:21371/api/queue/worker/status', {
      ignoreHTTPSErrors: true
    });
  ...
  test('should start and stop worker', async ({ request }) => {
    // Get initial status
    const statusResponse = await request.get('https://localhost:21371/api/queue/worker/status', {
      ignoreHTTPSErrors: true
    });
    ...
    if (!initialStatus.isRunning) {
      // Start worker
      const startResponse = await request.post('https://localhost:21371/api/queue/worker/start', {
        ignoreHTTPSErrors: true
      });
      ...
      // Verify it's running
      const runningStatus = await request.get('https://localhost:21371/api/queue/worker/status', {
        ignoreHTTPSErrors: true
      });
      ...
      // Stop worker
      const stopResponse = await request.post('https://localhost:21371/api/queue/worker/stop', {
        ignoreHTTPSErrors: true
      });
      ...
      await request.post('https://localhost:21371/api/queue/worker/stop', {
        ignoreHTTPSErrors: true
      });
  ...
  test('should handle starting already running worker', async ({ request }) => {
    ...
    if (!status.isRunning) {
      await request.post('https://localhost:21371/api/queue/worker/start', {
        ignoreHTTPSErrors: true
      });
    }
    
    // Try to start again
    const response = await request.post('https://localhost:21371/api/queue/worker/start', {
      ignoreHTTPSErrors: true
    });
  ...
  test('should handle stopping already stopped worker', async ({ request }) => {
    ...
    // Try to stop again
    const response = await request.post('https://localhost:21371/api/queue/worker/stop', {
      ignoreHTTPSErrors: true
    });
  ...
  test('should handle 404 for non-existent queue item', async ({ request }) => {
    const response = await request.get('https://localhost:21371/api/queue/items/non-existent-id', {
      ignoreHTTPSErrors: true
    });
  ...
  test('should handle cancel queue item', async ({ request }) => {
    ...
    const response = await request.post('https://localhost:21371/api/queue/items/fake-id/cancel', {
      ignoreHTTPSErrors: true
    });
  ...
  test('should handle retry queue item', async ({ request }) => {
    ...
    // For now, we test the error case
    const response = await request.post('https://localhost:21371/api/queue/items/fake-id/retry', {
      ignoreHTTPSErrors: true
    });
